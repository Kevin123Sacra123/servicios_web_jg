...
async function loadCitas() {
  const { data } = await axios.get('http://localhost:4000/citas/dental')
  return data;
}
        ...
        <div className="flex min-h-screen bg-gray-100">
            ...
            <section className="bg-gray-100 font-sans">
                <div className="max-w-6xl mx-auto p-6">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Dental</h1>
                        <p className="text-sm text-gray-500">Gestiona tus citas y revisa las recomendaciones de tus especialistas.</p>
                    </div>
                    <div className="flex gap-3">
                            <Link href={'/estudiante/dental/citas'} className="bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-800">+ Solicitar Nueva Cita</Link>
                    </div>
                    </div>
                    <div className="grid grid-cols-3 gap-6">
                        <div className="col-span-2 bg-white rounded-lg shadow p-5">
                            ...
                            <div>
                                {citas.map(cit => (
                                <div key={cit.id_citas}  className="flex justify-between items-center border rounded-lg p-4 mb-3">
                                    <div className="flex gap-3 items-center">
                                        <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                                            <TbDental className="text-blue-900 text-2xl" />
                                        </div>
                                        <div>
                                            <p className="font-semibold text-gray-800">{cit.personal}</p>
                                            <p className="text-xs text-gray-500">{cit.fecha} • {cit.hora}</p>
                                        </div>
                                    </div>
                                    <span className="bg-yellow-100 text-yellow-700 text-xs px-3 py-1 rounded-full">{cit.estado}</span>
                                </div>
                                ))}
                            </div>
                        ...
                        <div className="bg-white rounded-lg shadow p-5">
                            <h2 className="font-semibold text-gray-800 mb-4">Comentarios del Dentista</h2>
