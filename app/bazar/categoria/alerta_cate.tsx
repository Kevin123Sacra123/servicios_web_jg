...
async function deleteProduct(id_categoria) {
    try {
        await axios.delete(`http://localhost:4000/bazar/categoria/${id_categoria}`);
    } catch (error) {
        console.error(error);
    }
}

function Alerta_categoria({id, nombre}) {
    return(
        <AlertDialog>
            ...
            <AlertDialogHeader className="">
                ...
                <p className="text-red-500 text-2xl ">¿Estas seguro de eliminar esta categoria? </p>
                <p className="text-red-500 text-center text-xl">{nombre}</p>
                </AlertDialogTitle>
                <AlertDialogDescription className="text-left text-lg">
                Esta acción no se puede deshacer.
                ¿Estás seguro de que deseas eliminar permanentemente este categoria del inventario?
...
export default Alerta_categoria;